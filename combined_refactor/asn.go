...
func getASNReader() (*maxminddb.Reader, error) {
	...
	asnFullPath := filepath.Join(downloadDir, asnDBFile)
	if _, err := os.Stat(asnFullPath); os.IsNotExist(err) {
		...
		if err := atomicWriteFile(asnFullPath, content, 0644); err != nil {
			asnLookupState.err = err
			sendLog("保存 ASN 数据库失败: " + err.Error())
			return nil, err
		}
	}
	reader, err := maxminddb.Open(asnFullPath)
	if err != nil {
		asnLookupState.err = err
