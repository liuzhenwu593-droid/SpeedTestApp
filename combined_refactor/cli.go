...
var (
	...
	cliCommonFlags = []cliFlagInfo{
		...
		{name: "config", description: "CLI 配置文件路径，不存在时在二进制目录自动生成模板", defaultValue: "二进制目录/大绿龙优选-config.json"},
...
func newCLIConfigTemplate(cfg cliFileConfig) cliExportConfigTemplate {
	return cliExportConfigTemplate{
		...
		Config:          cfg,
		Description:     "大绿龙优选 CLI 全量配置；真正配置项在 config 内。",
...
func defaultCLIConfigPath() string {
	...
	if err != nil || exe == "" {
		return "大绿龙优选-config.json"
	}
	return filepath.Join(filepath.Dir(exe), "大绿龙优选-config.json")
}
...
func printCLIConfig(cfg *cliConfig) {
	...
	fmt.Printf("%s %s\n", colorize("大绿龙优选 版本:", ansiBold+ansiGreen), appVersion)
	...
	printGroup("通用参数", []item{
		...
		{"config", lookupCLIFlagDescription(cliCommonFlags, "config"), cfg.export.ConfigFile, "二进制目录/大绿龙优选-config.json"},
...
func printCLIUsage() {
	fmt.Fprintf(flag.CommandLine.Output(), "%s\n", colorize("大绿龙优选 命令行帮助", ansiBold+ansiGreen))
